...
async def submit_answers(
    ...
    try:
        ...
        if knowledge_count:
            # 1. 找出出现次数最大值
            max_count = max(knowledge_count.values())
            # 2. 筛选出所有“出现次数=最大值”的知识点（若有多个并列，默认取第一个）
            main_knowledges = [kid for kid, count in knowledge_count.items() if count == max_count]
            main_knowledge = main_knowledges[0]  # 取第一个作为“最主要知识点”
            main_stats = knowledge_stats[main_knowledge]  # 该知识点的答题详情
            
            # 3. 仅更新“最主要知识点”的掌握度
            # 获取或创建该知识点的学习记录
            record = db.query(LearningRecord)\
                .filter(
                    LearningRecord.student_id == current_user.id,
                    LearningRecord.knowledge_id == main_knowledge
                ).first()
            
            old_value = 0.0  # 默认初始掌握度
            if record:
                old_value = record.progress
            else:
                record = LearningRecord(
                    student_id=current_user.id,
                    knowledge_id=main_knowledge,
                    progress=old_value
                )
                db.add(record)
            
            # 计算掌握度变化
            question_count = len(main_stats)
            total_weight = sum(difficulty for _, difficulty, _ in main_stats)
            change = 0.0
            # 时间系数（基于预期时间调整）
            expected_time = total_weight * 120  # 难度总和×120秒为最大无惩罚时间
            if time_spent <= expected_time:
                time_factor = 1.0
            else:
                max_time_spent = expected_time * 3
                time_factor = 1.0 - 0.4 * (min(time_spent, max_time_spent) - expected_time) / (max_time_spent - expected_time)  # 最多惩罚40%
            
            for is_correct, difficulty, time_spent in main_stats:
                # 难度权重
                question_weight = difficulty / total_weight if total_weight > 0 else 1/question_count
                
                # 综合计算变化量
                if is_correct:
                    change += 0.4 * question_weight * time_factor  # 只对正确情况进行超时惩罚
                else:
                    change -= 0.2 * question_weight
            
            # 限制单次最大变化
            max_single_change = 0.4
            change = max(-max_single_change, min(max_single_change, change))
            
            # 计算新掌握度
            new_value = round(max(0, min(1, old_value + change)), 4)
            record.progress = new_value
