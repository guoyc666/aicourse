...
def understand_image(image_path, question):
    global answer
    answer = ""  # 重置全局变量，存储图片理解结果
    img_base64 = str(base64.b64encode(open(image_path, 'rb').read()), 'utf-8')
    text = [
        {"role": "user", "content": img_base64, "content_type": "image"},
        {"role": "user", "content": question}
    ]

    main(appid, api_key, api_secret, imageunderstanding_url, text)
    return answer

if __name__ == '__main__':

    info = understand_image("data/uploads/20251015-214642.png", "请总结图片内容")
    print("\n=== 图片理解结果 ===")
    print(info)
