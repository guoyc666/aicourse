...
export const masteryAPI = {
  ...
  async fetchAllMastery() {
    const res = await api.get("/api/mastery/all");
    return res;
  },
  // 获取某知识点的平均掌握度
  async fetchAverageMastery(knowledgeId: string) {
    const res = await api.get("/api/mastery/average", {
      params: { knowledge_id: knowledgeId }
    });
    return res;
  },
  ...
  async fetchAllAverageMastery() {
    const res = await api.get("/api/mastery/average/all");
    return res;
  },
  // 获取某知识点所有学生的掌握度列表
  async fetchMasteryList(knowledgeId: string) {
    const res = await api.get("/api/mastery/list", {
      params: { knowledge_id: knowledgeId }
    });
    return res;
  },
  ...
  async fetchAllMasteryList() {
    const res = await api.get("/api/mastery/list/all");
    return res;
  },
  // 获取模拟数据
